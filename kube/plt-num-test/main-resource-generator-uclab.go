...
// worker
func NewWorkerService(area Area, index int) Resource {
	name := "worker" + strconv.Itoa(area.Id+index)
	service := Resource{
		ApiVersion: "v1",
		Kind:       "Service",
...
func NewWorker(area Area, index int) Resource {
	name := "worker" + strconv.Itoa(area.Id+index)
	worker := Resource{
		ApiVersion: "v1",
		Kind:       "Pod",
			...
			Containers: []Container{
				...
				},
				{
					...
					Env: []Env{
						...
						{
							Name:  "SERVER_NAME",
							Value: "SynerexServer" + strconv.Itoa(area.Id+index),
						},
					},
				...
				},
				{
					...
					Env: []Env{
						...
						{
							Name:  "PROVIDER_NAME",
							Value: "WorkerProvider" + strconv.Itoa(area.Id+index),
						},
					},
				...
				},
				{
					...
					Env: []Env{
						...
						{
							Name:  "PROVIDER_NAME",
							Value: "AgentProvider" + strconv.Itoa(area.Id+index),
						},
					},
				},
			},
...
func main() {
	...
	option := Option{
		FileName: "main-worker-20-uclab.yaml",
		AreaCoords: []Coord{
...
func createData(option Option) []Resource {
	...
	for i := 0; i < workerNum; i++ {
		rsrcs = append(rsrcs, NewWorkerService(areas[0], i))
		rsrcs = append(rsrcs, NewWorker(areas[0], i))
	}
